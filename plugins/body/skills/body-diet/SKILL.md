...
compatibility:
  ...
  - tool: mcp__plugin_body_yazio__get_user_consumed_items
  - tool: mcp__plugin_body_yazio__get_user_water_intake
---
...
- For direct diet-only trend questions, pull the last 7-14 days, or 30 days when the question is broader. For a cadence review spanning a full month, sample **at least 8-10 dates**, not 4-5 — a sparse sample has previously made intermittent-but-real logging look like a complete stop when a denser re-check showed otherwise.
...
- `steps` and `activity_energy` in `get_user_daily_summary` sync automatically from a connected health app, independent of food logging — a day with real steps can still have zero meals logged. Don't infer "day was tracked" from steps/activity_energy alone; check the `meals` block itself. `get_user_exercises` is typically empty (Garmin is the real exercise log, not Yazio) — don't expect a separate workout history here.
- `get_user` (profile) is worth a one-time pull for context: registration date explains any "zero data" period before the account existed, and self-reported `activity_degree` feeds the calorie-goal calculation shown in every daily summary.
