...
- `oura-mcp` - sleep (score AND raw duration/HRV via `get_sleep`, not just `get_daily_sleep`), readiness, HRV, activity
- `garmin-mcp` - training load, workouts, VO2max (only visible via `get_activities` with `includeSummaryOnly: false`), steps. Body Battery/native stress/hydration have returned "service unavailable" for this account — don't assume they'll work.
- `withings-mcp` - weight, body composition, and pulse-wave-velocity/vascular-age (a second cardiovascular-age source alongside Oura)
