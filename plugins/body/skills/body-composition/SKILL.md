...
compatibility:
  ...
  - tool: mcp__withings__withings_get_measurements
...
- withings-mcp — weight kg, body fat %, muscle mass kg, bone mass kg,
  water %, BMI. Use `get_measurements` with a **broad `measureTypes` list**, not just weight/fat/muscle/bone — types 91 (Pulse Wave Velocity, m/s) and 155 (Vascular Age, years) are also on this scale and give a second, independent cardiovascular-age reading, cross-checkable against Oura's `get_daily_cardiovascular_age`. Raw PWV values roughly agree between the two devices; their age-conversion formulas do not (expect a 5-7 year gap) — compare the PWV number, not either device's "age" label, when reporting agreement or disagreement. Don't filter parsed measurement groups to type==1 (weight) only — same-visit PWV/vascular-age/heart-pulse readings can live in a separate group at the identical timestamp and get silently dropped if you do. `withings_get_users` reports the lifetime total measurement count for the device owner — useful to sanity-check whether a review window's reading count is a small fraction of total history (expected — most history predates the window) versus genuinely sparse relative to the account's normal cadence.
